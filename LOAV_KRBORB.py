...
# Add a panel to the game that has UI
#panel = canvas.add_panel()

# Position of the panel
#rect = panel.rect_transform
#rect.size = (300, 200)
#rect.postion = (110-(screen_size[1]/2), 0)
...
stg3_sol = conn.add_stream(stage_3_resources.amount, 'SolidFuel')
...
while True:
    ...
    # Separate Launch stage
    if not lstg_separated:
        if stg2_fuel() < 0.1:
            vessel.control.activate_next_stage()
            lstg_separated = True
            print('Launch Stage separated')
...
# If main acent stage isn't separated at time of apoapsis do it
if not lstg_separated:
    vessel.control.throttle = 0.0
    vessel.control.activate_next_stage()
    lstg_separated = True
